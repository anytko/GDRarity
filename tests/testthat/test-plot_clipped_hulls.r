...
test_that("plot_clipped_hulls handles empty input", {
  # Test case 1: Empty list input
  empty_input <- list()
  expect_silent(plot_clipped_hulls(empty_input))
})

test_that("plot_clipped_hulls handles valid input with multiple species", {
  ...
  map <- suppressWarnings(plot_clipped_hulls(clipped_convex_hulls))
...
test_that("plot_clipped_hulls generates a map with proper colors for multiple species", {
  # Create mock data for multiple species
  clipped_convex_hulls <- list(
    Species_A = list(st_sfc(st_polygon(list(rbind(c(0,0), c(0,1), c(1,1), c(0,0)))))),
    Species_B = list(st_sfc(st_polygon(list(rbind(c(1,1), c(1,2), c(2,2), c(1,1))))))
...
test_that("plot_clipped_hulls handles NULL geometries correctly", {
