...
species_range_size <- calc_range_size(data_frame = trait_df, min_points = 4, gbif_limit = 2250, num_cores = 1)
```
...
clipped_hulls <- clip_polygons_to_land(polygons_list = convex_hulls, continent_sf = continent_bounds)
```
...
acer_maps <- range_poly_map(data_frame = trait_df_subset)
```
